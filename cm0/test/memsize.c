...
unsigned char key_a[32], key_b[32];
  ...
  print_stack("RAM usage of keygen",-1,ctr);


  WRITE_CANARY(&a);
  newhope_sharedb(key_b,&sk,&sk);
  ctr = MAXSTACK - stack_count(canary,&a);
  print_stack("RAM usage of sharedb",-1,ctr);

	WRITE_CANARY(&a);   
	newhope_shareda(key_a,&sk,&sk);
    ctr = MAXSTACK - stack_count(canary,&a);
  print_stack("RAM usage of shareda",-1,ctr);
  ...
  newhope_sharedb(key_b,&sk,&sk);
	newhope_keygen(&sk,&temp);
    ctr = MAXSTACK - stack_count(canary,&a);
  print_stack("RAM usage of KEM",-1,ctr);
